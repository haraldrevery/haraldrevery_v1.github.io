...
const createPlexus = (canvas, isBackground, config) => {
    ...
    if (canvas.transferControlToOffscreen) {
        ...
        worker.postMessage({ type: 'init', canvas: offscreen, width: canvas.width, height: canvas.height, config, isDark: isDark() }, [offscreen]);
        ...
        return { type: 'worker', worker, cleanup: () => { window.removeEventListener('mousemove', mouseHandler); worker.terminate(); } };
    } else {
        // FALLBACK: Optimized Main Thread version (Last 10 years compatibility)
        const ctx = canvas.getContext('2d');
        let particles = [], startTime = Date.now();
        const cols = Math.ceil(canvas.width / config.lineDistance), rows = Math.ceil(canvas.height / config.lineDistance);
        ...
        const frame = () => {
            ...
            while (particles.length < target) {
                particles.push({ id: particles.length, x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random()-0.5)*(config.baseSpeed||0.8), vy: (Math.random()-0.5)*(config.baseSpeed||0.8) });
            }
            ...
            const binCount = isBackground ? 6 : 11, bins = Array.from({ length: binCount }, () => new Path2D()), dSqMax = config.lineDistance**2;
            particles.forEach(p1 => {
                const c = Math.floor(p1.x/config.lineDistance), r = Math.floor(p1.y/config.lineDistance);
                for(let i=-1; i<=1; i++) for(let j=-1; j<=1; j++) {
                    const nc=c+i, nr=r+j; if(nc>=0 && nc<cols && nr>=0 && nr<rows) grid[nc+nr*cols].forEach(p2 => {
                        if(p1.id >= p2.id) return;
                        const dx=p1.x-p2.x, dy=p1.y-p2.y, dSq=dx*dx+dy*dy;
                        if(dSq < dSqMax) {
                            const alphaBin = Math.floor((1 - Math.sqrt(dSq)/config.lineDistance)*(binCount-1));
            ...
            for(let b=0; b<binCount; b++) { ctx.strokeStyle = `rgba(${rgb}, ${isBackground ? config.lineOpacity : (b*8)/100})`; ctx.stroke(bins[b]); }
        ...
        return { type: 'fallback', cleanup: () => { window.removeEventListener('mousemove', mouseHandler); cancelAnimationFrame(handle.id); } };
    }
};
...
window.restartPlexus = function() {
    ...
    if (canvas) {
        canvas.width = 1000; canvas.height = 1100;
        logoInstance = createPlexus(canvas, false, { particleCount: window.innerWidth < 768 ? 0 : 500, lineDistance: 145, particleSize: 2 });
    }
};
    ...
    Alpine.data('plexusBackground', () => ({
        ...
        init() {
            ...
            this.instance = createPlexus(canvas, true, {
                particleCount: isMobile ? 25 : 120, lineDistance: isMobile ? 250 : 221,
                particleSize: 1.59, baseSpeed: 0.4, lineOpacity: 0.2, isBackground: true
            });
        }
    }));
});
...
window.restartLogoAnimations = function() {
    const lg = document.querySelector('#logo-shape-definition.animate-logo'); if (!lg) return;
    const lp = lg.querySelectorAll('path'), wv = document.querySelectorAll('.wave-echo');
    lp.forEach(p => { p.style.animation = 'none'; p.style.strokeDashoffset = '4000'; p.style.fillOpacity = '0'; });
    wv.forEach(w => { w.style.animation = 'none'; w.style.transform = 'scale(5)'; w.style.opacity = '0'; });
    void lg.offsetWidth;
    lp.forEach(p => p.style.animation = 'logoDraw 5s cubic-bezier(.75,.03,.46,.46) forwards');
    wv.forEach((w, i) => { setTimeout(() => w.style.animation = 'implodingWave 3.5s cubic-bezier(0.19, 1, 0.22, 1) forwards', i * 100); });
};

document.addEventListener('DOMContentLoaded', () => { window.restartPlexus(); setTimeout(window.restartLogoAnimations, 1); });
